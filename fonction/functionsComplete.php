<?php
    ...
    function getMenu($restoId){
        $connect = startConnection();
        //recherche des resto  dans la base de donnees
        $query = $connect->query("SELECT * FROM menu WHERE idRestau=$restoId");
        if($query->rowCount() > 0) //verifiation que le resto existe        
        {
            // stopConnection($connect);
            // return $query->fetch_all(MYSQLI_ASSOC);
    ...
    function addResto($username,$images,$addresse,$owner,$email,$services,$others)
     ...
     function addMenu($name,$prix,$resto)
     {
        //appel de la fonction de connexion
        $connect = startConnection();
       
        //enregistrement du resto dans la base de donnees
        $query = $connect->query("INSERT INTO menu (libelle,prix_total, idRestau) VALUES('$name','$prix','$resto')") or die("Erreur lors de l'ajout du restaurant");
        // stopConnection($connect);
        return true;
     }
